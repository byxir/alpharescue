...
export default function Example() {
  const [activePostBatch, setActivePostBatch] = useState(posts[0]?.batch);
  return (
    <div className="bg-sidebarBg py-24 font-montserratRegular sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="my-12 flex justify-center">
          <Tabs changeActivePosts={(id: number) => setActivePostBatch(id)} />
        </div>
        <div className="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          {posts
            .filter((p) => p.batch === activePostBatch)
            .map((post) => (
              <article
                key={post.title}
...
function Tabs({
  ...
  changeActivePosts: (id: number) => void;
}) {
